# encoding: utf-8
# Last Change:  2022-07-25 01:16:34
...
class MultiSegHead(nn.Module):
    ...
    def __init__(self, num_parts, tau=10):
        ...
        self.tau = tau
        self.seg_head = SegHeadFC(num_parts=num_parts, in_c=1024)
...
class GSS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if "part" in self.branch_lst:
            ...
            # num_parts = bg + parts
            self.part_branch = MultiSegHead(num_parts=self.n_parts + 0, tau=1.)  # (b bg+num_parts h w)
    ...
    def forward(self, batched_inputs):
        ...
        if self._cfg.MODEL.BACKBONE.WITH_NLKEY:
            fg_part_prob_maps = torch.cat([fg_map, part_prob_maps[:, 0:, :, :]], dim=1).detach()
        ...
        if hasattr(self, "part_branch"):
            for idx in range(0, part_prob_maps.size(1)):  # 0 indicates background
        ...
        if self.training:
            ...
            if hasattr(self, "part_branch"):
                ...
                for i in range(0, cp_part_prob_maps.size(1)):
                    _cur = cp_part_prob_maps[:, i:i + 1, :, :]
                    _stripe = horizontal_stripe(_cur, tau=0.1)  # (b h w)
            ...
            if hasattr(self, "part_branch"):
                part_prob_maps = prob_to_binary(part_prob_maps)
                part_prob_maps = part_prob_maps[:, 0:, :, :].float()  # (b num_parts h w)
            outputs = self.heads(branch_feat_map, part_prob_maps=part_prob_maps)
            ################################
            ...
            #
            #output_prob_map = global_norm + norm5 + norm2
            #
            #outputs.update({
            #    'prob_maps': output_prob_map
            #})
            return outputs
    ...
    def losses(self, outs):
        ...
        if hasattr(self, "part_branch"):
            ...
            #if self._cfg.MODEL.LOSSES.OCCLUDED.ENABLED:
            #    part_loss_func = self.part_loss_with_gt_occlude
            #else:
            #    part_loss_func = self.part_loss_with_gt
            ...
            ### Stripe-wise 
            loss_sr, prob_stripes = self.part_loss_stripe_wise(outs["part_prob_maps"][:,0:,:,:])
    ...
    def part_loss_stripe_wise(self, part_prob_maps):
        ...
        """
        #bs, h, w = part_prob_maps.size(0), part_prob_maps.size(2), part_prob_maps.size(3)
        #bg_stripes = torch.ones([bs,h,w], device=part_prob_maps.device)
        #bg_stripes[:,3:-3,:] = 0
        ...
        #full_prob_stripes = torch.cat([prob_stripes, bg_stripes.unsqueeze(0)], dim=0)
    ...
    def part_loss_pixel_wise(self, prob_stripes, part_prob_maps, fg_mask):
        ...
        seg_label = prob_stripes*fg_mask   # (b num_parts h w)
        ...
        # KLDiv loss
        log_part_prob_maps = torch.log(part_prob_maps[:,0:,:,:])
        loss_pr = F.kl_div(log_part_prob_maps, F.softmax(seg_label*1.0, dim=1)[:,0:,:,:])  # temperature=1

        # MSE loss
        #loss_pr = F.smooth_l1_loss(part_prob_maps, F.softmax(seg_label*1.0, dim=1))

        # Avoid trivial solution
        #cluster_prob = part_prob_maps[:,1:,:,:].sum(-1).sum(-1)  # (b num_parts)
        #cluster_prob = cluster_prob / cluster_prob.sum(-1, keepdim=True)
        #K = torch.tensor(float(cluster_prob.size(1)))
        #loss_trivial = (cluster_prob*torch.log(cluster_prob)).sum(1) + torch.log(K)
        #loss_trivial = loss_trivial.sum()
    ...
    def part_loss_with_gt_occlude(self, fg_prob_map, prob_maps, fg_gt):
        """
        Args:
            fg_prob_map: (b 1 h w)
        ...
        # loss_op = map_overlap_activ_penalty_occlude(prob_stripes, prob_maps[:, 1:, :, :].transpose(0, 1))

        # 3. segment parts
        prob_stripes = prob_stripes.detach().permute(1, 0, 2, 3)  # (b num_parts-1 h w)
        seg_label = torch.cat([1 - fg_gt, prob_stripes * fg_gt], dim=1)  # (b num_parts h w)
