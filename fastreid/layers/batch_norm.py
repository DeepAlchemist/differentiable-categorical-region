...
from fastreid.layers.pooling import GlobalSumPool2d
...
class BIN(nn.Module):
    ...
    def forward(self, x):
        '''
        Args:
            x(4D tensor): feature map
        Returns:
            output(4D tensor): feature map after normalizing
        '''
        feat_vect = self.pool_layer(x).flatten(start_dim=1)  # (b, dim_feat)
        batch_mean = torch.mean(feat_vect, dim=0)
        batch_var = torch.var(feat_vect, dim=0)

        if self.training:
            with torch.no_grad():  # TODO(note): important
        ...
        output = (x - mean) / (var ** .5 + self.eps)
        output = self.weight[None, :, None, None] * output

        if self.bias is not None:
            output = self.bias[None, :, None, None] + output
