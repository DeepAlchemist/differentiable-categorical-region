...
# Last Change:  2022-08-28 17:49:43
...
from fastreid.data import build_reid_test_loader, build_reid_train_loader, build_reid_vis_loader
from fastreid.evaluation import (DatasetEvaluator, ReIDEvaluator, PartialReIDEvaluator,
...
def default_argument_parser():
    ...
    parser.add_argument("--vis-only", action="store_true", help="perform visualization only")
...
class DefaultTrainer(SimpleTrainer):
    ...
    @classmethod  # todo-add:20201026
    def build_vis_loader(cls, cfg):
        """
        Returns:
            iterable
        It now calls :func:`fastreid.data.build_detection_train_loader`.
        ...
        """
        logger = logging.getLogger(__name__)
        logger.info("Prepare training set for visualizing")
        return build_reid_vis_loader(cfg)

    @classmethod
    def build_evaluator(cls, cfg, dataset_name, partial_eval=False, output_dir=None):
        data_loader, num_query = cls.build_test_loader(cfg, dataset_name)
        evaluator = PartialReIDEvaluator if partial_eval else ReIDEvaluator
        return data_loader, evaluator(cfg, num_query, output_dir)
    ...
    def test(cls, cfg, model):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            ...
            try:
                data_loader, evaluator = cls.build_evaluator(cfg, dataset_name, partial_eval=True)
    ...
    @classmethod  # TODO-Add@20201026
    def vis(cls, cfg, train_items, pid_to_lbl, model):
        """
        Args:
            cfg (CfgNode):
            train_items (List):
        ...
        Returns:
            dict: a dict of result metrics
        """
        from fastreid.utils.activ_map import ClassActivMap
        _factory = {
            'Market1501': 'mk'
        }
        logger = logging.getLogger(__name__)

        vis_dataset = 'Market1501'
        assert vis_dataset in cfg.DATASETS.NAMES
        vis_pids = [10, 11, 23, 27, 28, 64, 93, 901]
        vis_pids = list(map(lambda x: vis_dataset.lower() + "_" + str(x), vis_pids))
        save_dir = './logs/{}/'.format(_factory[vis_dataset])
        CAM = ClassActivMap(
            model,
            train_items,
            pid_to_lbl,
    ...
    @classmethod  # TODO-Add@20210118
    def prob_maps(cls, cfg, image_items, pid_to_lbl, model):
        """
        Args:
            cfg (CfgNode):
            image_items (List):
        ...
        Returns:
            dict: a dict of result metrics
        """
        from tests.probmap_test import plot_prob_maps, plot_multilevel_prob_maps, plot_fg_maps, save_fg_maps
        _factory = {
            'Market1501': 'market',
            'OccludedDukeMTMC': 'occluded_duke',
            'DukeMTMC': 'duke'
        }

        vis_dataset = 'DukeMTMC'
        assert vis_dataset in cfg.DATASETS.NAMES
        #vis_pids = [10, 11, 23, 27, 28, 64, 93, 901]
        #vis_pids = list(map(lambda x: vis_dataset.lower() + "_" + str(x), vis_pids))
        save_dir = './logs/{}/partProbMap'.format(_factory[vis_dataset])

        #save_fg_maps(model, image_items, cfg.INPUT.SIZE_TRAIN, save_dir)
        #plot_prob_maps(model, image_items, cfg.INPUT.SIZE_TRAIN, save_dir)
        plot_multilevel_prob_maps(model, image_items, cfg.INPUT.SIZE_TRAIN, save_dir)
        return
