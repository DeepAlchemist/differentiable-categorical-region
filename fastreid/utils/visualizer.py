...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            for i in range(max_rank):
                ...
                ### debug ###
                p_id = os.path.basename(query_info['img_paths']).split('_')[0]
                p_cam_id = os.path.basename(query_info['img_paths']).split('_')[1]
                g_id = os.path.basename(gallery_info['img_paths']).split('_')[0]
                g_cam_id = os.path.basename(gallery_info['img_paths']).split('_')[1]
                # import pdb; pdb.set_trace()
                if p_id == g_id and p_cam_id != g_cam_id:
                    labellabel = 1
                else:
                    labellabel = 0
                #############
                ...
                ax.set_title(f'{self.sim[q_idx, sort_idx[i]]:.3f}/{label}/cam{cam_id}/{labellabel}')
                ax.axis("off")
            # if actmap:
            #     act_outputs = []
            #
    ...
    def vis_rank_list(self, output, vis_label, num_vis=100,
                      rank_sort="ascending", label_sort="ascending", max_rank=5,
                      actmap=False):
